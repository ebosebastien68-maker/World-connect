  ...
  return (
    <div className="article-card" data-article-id={article.article_id}>
          ...
          <div className="admin-options">
            ...
            {showAdminMenu && (
              <div className="options-menu show">
                <a onClick={() => onNavigate(`/edit-article?id=${article.article_id}`)}>
                  <i className="fas fa-edit"></i> Modifier
                </a>
                <a className="delete" onClick={() => { setShowAdminMenu(false); onDelete(article.article_id); }}>
      ...
      <div className="article-content">
        ...
        {images.length > 0 && (
          <div className={`article-images ${imageClass}`}>
            {images.map((img, i) => (
              <img key={i} src={img.image_url} alt="Image" loading="lazy"
        ...
        {(article.texte_url || article.vente_url || article.whatsapp_url) && (
          <div className="article-links">
            {article.texte_url && (
              <button className="link-btn" onClick={() => window.open(article.texte_url, '_blank')}>
                <i className="fas fa-link"></i> Lien
              </button>
            )}
            {article.vente_url && (
              <button className="link-btn" onClick={() => window.open(article.vente_url, '_blank')}>
                <i className="fas fa-shopping-cart"></i> Acheter
              </button>
            )}
            {article.whatsapp_url && (
              <button className="link-btn" onClick={() => window.open(article.whatsapp_url, '_blank')}>
      ...
      <div className="reactions">
        {(['like', 'love', 'rire', 'colere'] as const).map((type) => {
          ...
          const counts: Record<string, number> = {
            like: article.reaction_like, love: article.reaction_love,
      ...
      <div className="comments-reactions-section">
        <div className="comments-toggle-large" onClick={() => setShowComments(!showComments)}>
      ...
      <div className={`comments-section ${showComments ? 'show' : ''}`} ref={commentsRef} />
    </div>
  );
};
...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 1; i < pos.length; i += 3) pos[i] += Math.sin(Date.now() * 0.001 + i) * 0.01;
  ...
  const requestNotificationPermission = useCallback(async (user: SupabaseUser) => {
  ...
  const loadArticles = useCallback(async () => {
    try {
      ...
      setDisplayedArticles(articles as Article[]);
      setEmptyState(false);
    } catch {
      setArticlesLoading(false);
  ...
  const loadUserReactions = useCallback(async (user: SupabaseUser) => {
    try {
      const { data } = await supabase.from('article_reactions').select('article_id, reaction_type').eq('user_id', user.id);
      if (!data) return;
      const map: Record<string, string[]> = {};
      data.forEach((r: { article_id: string; reaction_type: string }) => {
  ...
  const setupRealTimeUpdates = useCallback((user: SupabaseUser) => {
    ...
    supabase.channel('notifications-changes').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: `user_id=eq.${user.id}` }, () => loadNotificationCount(user)).subscribe();
  ...
  const handleReaction = useCallback(async (articleId: string, reactionType: string) => {
    ...
    try {
      ...
      if (existing) {
        ...
        setUserReactions((prev) => ({ ...prev, [articleId]: [...(prev[articleId] || []), reactionType] }));
      }
  ...
  const deleteArticle = useCallback(async (articleId: string) => {
    ...
    try {
      ...
      if (error) throw error;
      showToast('Suppression réussie', 'Article supprimé', 'success');
      await loadArticles();
    } catch {
  ...
  const handleShare = useCallback((platform: string, articleId: string) => {
    ...
    const text = encodeURIComponent(article ? article.texte.substring(0, 100) + '... - World Connect' : 'World Connect');
    ...
    if (platform === 'instagram' || platform === 'tiktok') {
      ...
      showToast(`Partage ${platform}`, 'Lien copié', 'info');
  ...
  const navigateToPage = useCallback((page: PageName) => {
    ...
    setCurrentPage(page);
    setMenuOpen(false);
    if (page !== 'home' && !loadedPages.has(page)) {
      const src = page === 'game' ? '/offline' : `/${page}`;
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (!target.closest('.navbar') && !target.closest('.menu-toggle-btn')) setMenuOpen(false);
    };
  ...
  return (
    <>
      ...
      <div className={`welcome-popup ${welcomeVisible ? 'show' : ''}`}>
        ...
        <div className="welcome-popup-name">{userProfile ? `${userProfile.prenom} ${userProfile.nom}` : ''}</div>
        <div className="welcome-popup-subtitle">Nous sommes disponibles à tous vos Services.</div>
      </div>

      <button className={`menu-toggle-btn ${menuOpen ? 'active' : ''}`} onClick={handleMenuToggle}>
      ...
      <nav className={`navbar ${menuOpen ? 'show' : ''}`}>
        ...
        <div className="navbar-content">
          ...
          ] as { page: PageName; icon: string; label: string; badge?: number }[]).map(({ page, icon, label, badge }) => (
            <div key={page} className={`nav-item ${currentPage === page ? 'active' : ''}`} onClick={() => navigateToPage(page)}>
      ...
      {currentPage === 'home' && (
        <button className={`search-toggle-btn ${searchPanelOpen ? 'active' : ''}`} onClick={() => setSearchPanelOpen(!searchPanelOpen)}>
          <i className="fas fa-search"></i><span>Recherche</span>
        </button>
      )}
      ...
      <div className="spa-container">
        <div className={`spa-page ${currentPage === 'home' ? 'active' : ''}`}>
          {articlesLoading && (
            <div className="loader"><div className="spinner"></div><p style={{ marginTop: 20 }}>Chargement...</p></div>
          )}
          {!articlesLoading && emptyState && emptyStateContent === 'offline' && (
            <div className="empty-state"><i className="fas fa-wifi-slash"></i><h3>Mode hors ligne</h3><p>Impossible de charger les articles</p></div>
          )}
          {!articlesLoading && emptyState && emptyStateContent === 'error' && (
            <div className="empty-state">
              <i className="fas fa-exclamation-triangle"></i><h3>Erreur de chargement</h3>
              <button onClick={loadArticles} style={{ marginTop: 20, padding: '12px 24px', background: 'var(--accent-kaki)', color: 'white', border: 'none', borderRadius: 12, cursor: 'pointer', fontWeight: 600 }}>
                <i className="fas fa-sync-alt" style={{ marginRight: 8 }}></i>Réessayer
              </button>
            </div>
          )}
          {!articlesLoading && emptyState && emptyStateContent === 'default' && (
            <div className="empty-state"><i className="fas fa-newspaper"></i><h3>Aucun article</h3><p>Revenez plus tard</p></div>
          )}
        ...
        {(['messages', 'notifications', 'plus', 'game'] as PageName[]).map((page) => (
          <div key={page} className={`spa-page ${currentPage === page ? 'active' : ''}`}>
            <div className="iframe-container">
              {iframeUrls[page] && <iframe className="page-iframe" src={iframeUrls[page]} title={pageLabels[page]} />}
            </div>
          </div>
        ))}
      </div>
